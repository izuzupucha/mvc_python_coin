...
class BBView:
    @staticmethod
    def show(controller):
        st.header(" Phân tích BB Indicator")
        with st.form("bb_form"):
            ...
            #    "Nhập chu kỳ BB (period)", min_value=5, max_value=100, value=14, step=1
            #)

            submitted = st.form_submit_button("🔍 Phân tích BB")
            ...
            if not coin_pair.strip():
                ...
                try:
                    ...
                    if result is not None:
                        ...
                        st.info(f"Không có dữ liệu trả về từ hàm phân tích BB {result}")
                except Exception as e:
                    st.error(f"Lỗi khi phân tích BB {e}")
        st.divider()
